...
Component({
  ...
  methods: {
                      ...
                      if (fileList && fileList.length) {
                        ...
                        commonRequest({
                          ...
                          success: (res) => {
                            ...
                            const { data } = res;
                            if (data && data.Result) {
                              void this.sendMessage(data.Result).catch((err) =>
                                console.error("sendMessage from voice failed", err)
                              );
                              resolve(data.Result);
                            } else {
                              resolve();
    ...
    sendMessage: async function (message) {
      ...
      if (chatMode === "model") {
        const { quickResponseModel, temperature, maxTokens } = modelConfig;
        const cloudInstance = await getCloudInstance(this.data.envShareConfig);
        ...
        const callData = {
          model: quickResponseModel || "hunyuan-lite",
          messages: requestMessages,
        };
        if (typeof temperature === "number") {
          callData.temperature = temperature;
        }
        if (typeof maxTokens === "number" && maxTokens > 0) {
          callData.maxTokens = maxTokens;
        }
        try {
          const callRes = await cloudInstance.callFunction({
            name: "hunyuanChat",
            timeout: 60000,
            data: callData,
          });
          ...
          if (result.ok) {
            lastValue.content = result.content || "";
          } else {
            lastValue.content = result.error || this.data.defaultErrorMsg;
            lastValue.error = result.code ? `${result.code}: ${lastValue.content}` : lastValue.content;
          }
        } catch (e) {
        ...
        lastValue.record_id = "record_id" + String(+new Date());
